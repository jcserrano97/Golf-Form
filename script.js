...
document.addEventListener('DOMContentLoaded', function() {
                ...
                if (file) {
                    ...
                    reader.onload = function(e) {
                        logoPreview.innerHTML = `
                            ...
                            <p style="margin-top: 0.5rem; font-size: 0.9rem; color: #22C55E;">✓ Logo uploaded successfully</p>
                        `;
                    };
    ...
    function updateOrderSummary() {
        ...
        if (selectedProducts.length > 0) {
                ...
                if (product) {
                    if (product.custom) {
                        ...
                        summaryHTML += `
                            <div class="selected-product">
                                <span>${product.name}</span>
                                <span>$${product.price} × ${quantity} = $${productTotal.toLocaleString()}</span>
                            </div>
                        `;
                    }
                }
            ...
            summaryHTML += '</div>';
            summaryHTML += `<p style="font-weight: 600; font-size: 1.1rem; margin-top: 1rem; color: #02734E;">Estimated Total: $${totalEstimate.toLocaleString()}</p>`;
            summaryHTML += '</div>';
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                emailService.showMessage(
                    'Thank you! Your custom order requirements have been sent successfully. Our team will contact you within 24 hours.',
                    'success'
                );
